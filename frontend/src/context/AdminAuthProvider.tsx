...
export const AdminAuthProvider: React.FC<AdminAuthProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      console.log('AdminAuthProvider: Checking auth - token:', !!token, 'userData:', !!userData);
      ...
      if (token && userData) {
        try {
          ...
          if (parsedUser.role === 'Admin') {
            ...
            setUser(adminUser);
            console.log('AdminAuthProvider: Admin user restored from localStorage:', adminUser);
          } else {
            // User is not Admin, don't set user but don't clear data
            console.log('AdminAuthProvider: User is not Admin, not setting admin user');
          }
        } catch (error) {
        ...
        console.log('AdminAuthProvider: No token or user data found in localStorage');
      }
      console.log('AdminAuthProvider: Auth check completed, setting isLoading to false');
      setIsLoading(false);
    };
    ...
    try {
      ...
      if (response.success && response.data?.user) {
        ...
        if (response.data.user.role === 'Admin') {
          ...
          setUser(adminUser);
          console.log('AdminAuthProvider: Login successful, user saved:', adminUser);
      ...
      console.log('AdminAuthProvider: Login process completed, setting isLoading to false');
      setIsLoading(false);
    }
  };
